...
To mitigate some risks administrators must configure the surrounding systems correctly.
...
The \emph{Origin} Header sent by the Browser must match the domain on which the XMPP Broker application is hosted.
...
In our development setup, we use the second approach using Nginx as Openfire does not properly validate the \emph{Origin} header.