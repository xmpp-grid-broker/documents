...
As described in the corresponding architectural decision (Appendix~\fullref{sec:architectural-decisions}), the main reason for \gls{sasl-external} is its higher level of security and and its relatively simple scaling capabilities.
...
To circumvent this limitation, an HTTP reverse proxy (see section~\fullref{sec:implemented-web-application-topology}) might be used to handle \gls{tls} authentication.
