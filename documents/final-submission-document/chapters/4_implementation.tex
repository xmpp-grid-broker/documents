...
As our XMPP-Grid Broker implementation is intended to be a maintainable, production-ready application rather than a prototype, we placed much emphasis on code quality.
...
Another benefit of the Angular CLI project setup is that it comes with ``codelyzer'' (including ``tslint'') for static code analysis and style linting.

Apart from the built-in linting mechanism, we followed Angular's Style Guide.
Using the JetBrains IDEs (IntelliJ Ultimate and Webstorm) turned out to be particularly helpful as they give quick feedback for frequent mistakes and even violations of the Angular Style Guide.
...
Sadly, we were not able to find such tools that were actively maintained and work with TypeScript.tools for such (oo) code metrics such as do not exist.
...
Each change, for the documentation and code, was reviewed using GitHub pull-requests.
In most cases, minor changes were detected and addressed during these reviews.
Continuous integration with TravisCI ensured that these changes never contained compilation errors or failing tests.

We also regularly discussed architectural and structural questions in our retrospectives and standup meetings.

Summarising the above, it can be said that, writing clean, modular and testable code has been our priority.
