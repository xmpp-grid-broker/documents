...
In practice, most \gls{xmpp-grid-broker} deployments require an HTTP proxy in front of the \gls{xmpp} server as security measure (see section~\fullref{sec:implemented-web-application-topology}).
...
As outlined in section~\fullref{sec:limitations-of-requirement-multiple-administrators}, practical deployments of \glspl{xmpp-grid-broker} mostly use an HTTP proxy.
