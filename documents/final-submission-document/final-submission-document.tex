...
%Result
Our proposed architecture earmarks a purely client-side web application that communicates with the controller over WebSockets or HTTP streams (BOSH).
The controller is typically protected by a reverse proxy, which also hosts our application.
User logins are performed using mutual \gls{tls} authentication to conform to the IETF standard draft.
...
In the future, these improvements can be realised by enhancing the related XMPP standards or by specifying and implementing proprietary protocols.
\end{abstract}
...
\include{appendices/personal-reports}
