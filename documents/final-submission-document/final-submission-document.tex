...
%Goal
The goal of this thesis is to design and implement an XMPP-Grid Broker to configure existing Controllers (XMPP server), focussing on portability, extensibility and the security aspects in a production environment.
The  Broker application should enable administrators to configure XMPP-Grids in a usable and productive way.\\
\vspace*{1ex}

%Result
Our proposed architecture earmarks a purely client-side application that communicates typically through a regular reverse proxy with the XMPP Controller over WebSockets or HTTP streams (BOSH).
Authentication is performed using mutual TLS authentication (client certificates) to conform to the IETF standard draft.
...
These improvements can be realised in the future by enhancing the related XMPP standards or by specifying and implementing proprietary protocols
\end{abstract}
