...
% Context
Most enterprises depend on large, diverse IT infrastructures which must be monitored, updated and protected from numerous attack scenarios.

To address this problem, the IETF Managed Incident Lightweight Exchange~(MILE) working group proposes the standard ``Using XMPP for Security Information Exchange''. The standard describes how an XMPP-Grid could be used to exchange security-relevant information between network endpoints.\\
\vspace*{1ex}

%Problem
Today, no implementation of an interface specialised on administering XMPP-Grids exists, which is production ready and platform independent .\\
\vspace*{1ex}

%Goal
%TODO: SMART Goal
The goal of this thesis is to engineer a platform independent administration interface as extension to existing XMPP-Grid Broker backends. This interface should enable administrators to configure XMPP-Grids in a usable and productive way.

The main focus of the implementation should lie on portability, extensibility and security aspects of the application in a production environment.\\
\vspace*{1ex}


%Result
We propose the architecture of an XMPP-Grid Broker as addition to existing XMPP servers and present an implementation based on the Angular framework.

Our implementation enables administrators to create and configure communication Topics, apprehend the underlying hierarchy and manage permissions. Additionally, persistent items of Topics can be inspected and published.\\
\vspace*{1ex}
